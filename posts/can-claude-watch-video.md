...
- “Find where the checkout bug appears and describe what the user did immediately before it.”
- “Break down this public ad into hook, proof, offer, and call to action.”

`claude-real-video` supports that direction explicitly with a `--why` flag, according to its README. The point of the flag is simple: tell the tool why you are watching before the evidence is packaged. That makes the downstream model less likely to wander into a generic recap.

The second rule is to preserve the evidence. The README describes a `--kb` option that saves analysis into a notes folder. Whether you use that exact feature or a normal company folder, the principle is the same. If a video review creates an operational decision, keep the frames, transcript, prompt, and summary together. Do not let the answer float away from the source.

The third rule is to separate observation from judgment. First ask the model what is visible and audible. Then ask what it means. If those are mixed together too early, you get a confident interpretation before you know whether the evidence is right.
...
If your bottleneck is turning repeated work into managed systems, start with [/agentic-systems](/agentic-systems). If the video is part of lead response, landing-page review, or ad analysis, the same operating discipline applies to [/smb-marketing](/smb-marketing). BBH’s venture work sits at [/ventures](/ventures): proof that useful systems are built, tested, and improved in the open.
